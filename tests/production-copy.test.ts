...
describe("production copy and fixture isolation", () => {
  ...
  it("keeps Live Portfolio as the default, and Study Mode on design briefs", () => {
    ...
    expect(liveRender.indexOf("<PortfolioBenchmarkChart")).toBeLessThan(liveRender.indexOf("pf-risk"));
    expect(liveRender.indexOf("COMPARE_AGAINST_LABEL")).toBeGreaterThan(liveRender.indexOf("<PortfolioBenchmarkChart"));
    expect(liveRender.indexOf("pf-moves-block")).toBeGreaterThan(liveRender.indexOf("COMPARE_AGAINST_LABEL"));
    ...
    expect(read("src/components/PortfolioBenchmarkChart.tsx")).toContain("pf-benchmark-compare");
