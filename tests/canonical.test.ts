...
describe("getConvictionBadge", () => {
  it("returns correct badge for strong positive evidence", () => {
    const snapshot = buildConvictionSnapshot({
      ...
      insider: { events: makeInsiderEvents("purchase"), status: "success" },
      earnings: { ticker: "BULL", history: [], forecasts: [], gradeActions: [], historyScore: 25, revisionScore: 20, score: 22, momentum: "Estimates rising", nextEarningsDate: null, asOf: "2026-07-15", source: "nasdaq", status: "success" },
      political: null,
      historyPoints: [],
      quote: makeQuote(),
      week52High: null,
  ...
  it("returns correct badge for negative evidence", () => {
    const snapshot = buildConvictionSnapshot({
      ...
      institutional: { results: institutionalRows("Reduced", "Exited"), status: "success" },
      insider: { events: makeInsiderEvents("sale"), status: "success" },
      earnings: { ticker: "BEAR", history: [], forecasts: [], gradeActions: [], historyScore: -30, revisionScore: -25, score: -28, momentum: "Estimates falling", nextEarningsDate: null, asOf: "2026-07-15", source: "nasdaq", status: "success" },
      political: null,
      historyPoints: [],
