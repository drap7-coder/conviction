...
describe("portfolio Sector Mix", () => {
  it("uses a flat donut in an inset well that fills the card", () => {
    ...
    expect(chart).not.toContain("DEPTH_LAYERS");
    expect(chart).not.toContain("radialGradient");
    expect(chart).not.toContain("3d");
