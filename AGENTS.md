...
- Heatmaps share one tile language (no pulsing status dots, uniform tiles, brokerage teal/red fills from `src/lib/display/heat-color.ts`). **Quote grids** (Watchlist, Trending) show last `$` plus session `%`. **Pulse index/sector/theme heatmaps are `%` only** — those names are indexes/ETFs/countries, so an ETF share price in `$` next to a `%` chip reads as mixed units. Page heroes use `ProductStage` on canvas tokens (`var(--card)` / `var(--ink)`).
