...
- Heatmaps (Pulse indexes/sectors/themes, Trending “Active names”, Watchlist quotes) share one tile language: no pulsing status dots, last price when the quote has one, uniform tiles (no market-cap span), and brokerage teal/red fills from `src/lib/display/heat-color.ts`. Page heroes use `ProductStage` on canvas tokens (`var(--card)` / `var(--ink)`), not a separate light-shell + ink-header treatment.
