...
/**
 * Company "Today's read" — collapsed by default.
 * `/api/conviction/score` (and earnings/history) only run after the reader expands.
 */
export function CompanyDecisionBrief({ ticker }: { ticker: string }) {
  ...
  const showContent = showBody && (loading || loadedTicker === ticker);

  return (
    <details
      ref={detailsRef}
      id="todays-read"
      className="company-decision-disclosure company-driver-module"
      onToggle={(event) => setExpanded((event.currentTarget as HTMLDetailsElement).open)}
    >
      <summary className="company-decision-disclosure-summary">
        <span className="company-decision-disclosure-label">Today&apos;s read</span>
        <span className="company-decision-disclosure-copy">
          Score, earnings, and tape — open only when you want the analysis.
        </span>
      </summary>
