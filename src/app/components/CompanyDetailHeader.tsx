...
/**
 * First-viewport composition: who it is + what it costs.
 * Catalysts live in What’s driving the move; verdict lives in Conviction Signals.
 */
    ...
    <header className={`detail-header${showNav ? "" : " detail-header--embedded"}`}>
      ...
      <div className="cdh-body">
        <div className="cdh-identity">
          ...
          <div className="cdh-identity-copy">
            <h1 className="cdh-ticker">{ticker}</h1>
            <p className="cdh-name">
              {companyName}
              {sectorName ? (
                <>
                  <span className="cdh-name-sep" aria-hidden="true">·</span>
                  <span className="cdh-sector">{sectorName}</span>
                </>
              ) : null}
            </p>
          </div>
        ...
        <div className="cdh-prices" aria-label={`${ticker} price`}>
          ...
          <div className="cdh-live-price">
            {loading ? (
              ...
              <>
                <span className={`cdh-arrow ${direction}`}>{arrow}</span>
                <span className="cdh-price-big">${formatPrice(live.price)}</span>
                {live.label ? (
                  <span className="cdh-price-session">{live.label}</span>
                ) : null}
                {changeText ? (
                  <span className={`cdh-change ${direction}`}>
          ...
          {isExtendedSession && quote ? (
            <div className="cdh-session-row">
              <span className="cdh-session-ref">
                ...
                <span className="cdh-ref-price">${formatPrice(quote.price)}</span>
                {regularChangeText ? (
