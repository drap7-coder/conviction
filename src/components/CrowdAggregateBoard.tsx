...
export type CrowdAggregateMode = "held" | "watched";
...
/** Most Held / Most Watched aggregate boards — Portfolio market intelligence. */
export function CrowdAggregateBoard({ mode }: { mode: CrowdAggregateMode }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!res.ok) throw new Error("Could not load community holdings.");
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : "Could not load community holdings.");
        }
      } finally {
  ...
  useEffect(() => {
    ...
    async function hydrateViewer() {
      try {
        ...
        const watchEntries =
          watchAuthenticated && Array.isArray(watchData?.entries)
            ? (watchData.entries as Array<{ ticker: string }>)
            : [];
        setBookTickers(new Set(portfolio.positions.map((p) => p.ticker.toUpperCase())));
        setWatchTickers(
          new Set(
            watchAuthenticated
  ...
  return (
    <div className="crowd-aggregate-panel">
      {groups.length > 0 ? (
      ...
      <section className="surface-shell crowd-board" aria-label={mode === "held" ? "Most held" : "Most watched"}>
        <div className="crowd-board-head">
          <div className="crowd-board-title">
            <h2>{mode === "held" ? "Most held" : "Most watched"}</h2>
          </div>
        </div>
        ...
        <div className="surface-well crowd-well">
          {loading && !snapshot ? (
            ...
            <p className="crowd-empty">
              {mode === "held" ? "No holdings to rank yet." : "No watchlists to rank yet."}
            </p>
          ) : (
